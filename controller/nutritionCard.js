...
exports.displayNutritionCards = function getNutritionData(foodName, session){
    //api_key:YOURKEY
    var url = "https://api.nal.usda.gov/ndb/search/?format=json&q="+foodName+"&sort=r&max=1&offset=0&api_key=HEnVHhh8G1Nc55q5LpMGgG7BwWiO6fvRnFt7LwEI";
...
function getFoodNutrition(message, foodName, session){
    ...
    var ndbno = foodNutritionList.list.item[0].ndbno;
     //api_key:YOURKEY
    var url = "https://api.nal.usda.gov/ndb/reports/?ndbno="+ndbno+"&type=f&format=json&api_key=HEnVHhh8G1Nc55q5LpMGgG7BwWiO6fvRnFt7LwEI";
    rest.getNutritionData(url, session, foodName, displayNutritionCards);

}


//callback from rest.getNutritionData(url, session, foodName, displayNutritionCards); above
